...
class Config{
	...
	public function Root($data){
		...
		$cambio["cambio_nombre_precio"] = Encrypt::Encrypt($data["cambio_nombre_precio"],$_SESSION['secret_key']);
	    $cambio["time"] = Helpers::TimeId();
	...
	public function CrearVariables(){
		...
		if ($r = $db->select("*", "config_master", "WHERE td = ".$_SESSION['td']."")) { 
			...
			$_SESSION['cambio_nombre_precio'] = $root["cambio_nombre_precio"];
			...
			$_SESSION['cambio_nombre_precio'] = $encrypt->Decrypt(
			$_SESSION['cambio_nombre_precio'],$_SESSION['secret_key']);
