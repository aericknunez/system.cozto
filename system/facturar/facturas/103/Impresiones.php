...
$ip = Helpers::GetIp();
if($ip == "192.168.0.197"){
$this->ticketCaja1($efectivo, $numero);
//$this->Ticket2($efectivo, $numero);
}else{
  $this->ticketCaja2($efectivo, $numero);
}}


 public function TicketCaja1($efectivo, $numero){
  $db = new dbConn();
...
$printer -> setJustification(Printer::JUSTIFY_CENTER);
$printer->text("DFIGUEROA");
$printer->feed();
$printer->text("Distribuidora Figueroa");
$printer->feed();
$printer->text("Venta de bebidas (Sodas, Jugos, Cervezas y más)");
$printer->feed();
$printer->text("Telefono y WhatsApp 7203-0791");
$printer->feed();
$printer->text("6ta Av. norte entre 3ra calle oriente");
$printer->feed();
$printer->text("Barrio Nuevo Contiguo a Floristeria Angelita");
$printer->feed();
$printer->text("frente a Agroservicio Chaparral, Metapán");
...
public function ticketCaja2($efectivo, $numero){
  $db = new dbConn();
  $nombre_impresora = "POS-80";
  // $img  = "C:/AppServ/www/pizto/assets/img/logo_factura/abrego.jpg";
...
$printer -> setJustification(Printer::JUSTIFY_LEFT);
$printer -> setEmphasis(true);
$printer -> text($this->Item2("Cant", 'Producto', 'Precio', 'Total'));
$printer -> setEmphasis(false);
...
$printer -> text($this->Item2($b["cant"], substr($b["producto"], 0, 28), $b["pv"], $b["total"]));
...
// $printer -> text($this->DosCol("Sub Total " . $_SESSION['config_moneda_simbolo'] . ":", 40, Helpers::Format(Helpers::STotal($subtotalf, $_SESSION['config_imp'])), 10));
$printer -> text($this->DosCol2("Sub Total  " . $_SESSION['config_moneda_simbolo'] . ":", 30, Helpers::Format($subtotalf), 10));
...
$printer -> text($this->DosCol2("TOTAL " . $_SESSION['config_moneda_simbolo'] . ":", 30, Helpers::Format($subtotalf), 10));
...
$printer -> text($this->DosCol2("Efectivo " . $_SESSION['config_moneda_simbolo'] . ":", 30, Helpers::Format($efectivo), 10));

//cambio
$cambios = $efectivo - $subtotalf;
$printer -> text($this->DosCol2("Cambio " . $_SESSION['config_moneda_simbolo'] . ":", 30, Helpers::Format($cambios), 10));
...
$printer->feed();
$printer -> text($this->DosCol2("Fecha: ".$fechaf, 20, "Hora: ".$horaf, 20));
...
$printer -> text($this->DosCol("TICKET $: ", 20, Helpers::Format($t_ticket), 5));
$printer -> text($this->DosCol("FACTURA $: ", 20, Helpers::Format($t_factura), 5));
$printer -> text($this->DosCol("CREDITO FISCAL $: ", 20, Helpers::Format($t_credito), 5));
$printer -> text($this->DosCol("IMPUESTO $: ", 20, Helpers::Format($t_imp), 5));
$printer -> text($this->DosCol("TOTAL $: ", 20, Helpers::Format($total-$t_ticketN), 5));
...
$printer -> text($this->DosCol("EFECTIVO $: ", 20, Helpers::Format($t_efectivo-$t_ticketN), 5));
$printer -> text($this->DosCol("TARJETA DE CREDITO $: ", 19, Helpers::Format($t_tarjeta), 5));
$printer -> text($this->DosCol("TOTAL $: ", 20, Helpers::Format($total-$t_ticketN), 5));
$printer -> text($this->DosCol("APERTURA $: ", 20, Helpers::Format($caja_chica), 5));
$printer -> text($this->DosCol("EFECTIVO INGRESADO $: ", 19, Helpers::Format($efectivo), 5));
$printer -> text($this->DosCol("DIFERENCIA $: ", 20, Helpers::Format($diferencia), 5));
...
// VENTA TOTAL - subtotal, iva, total
  ...
  $printer -> text($this->Col4("VENTA TOTAL",0 ,  "SUBTOTAL", 10,  "IVA", 10, "TOTAL", 10));

  $printer -> text($this->Col4("TOTAL GRAVADO",0 ,  $st, 10,  $im, 10, $to, 10));
  $printer -> text($this->Col4("TOTAL EXENTO",0 ,  Helpers::Format(0), 10,  Helpers::Format(0), 10, Helpers::Format(0), 10));
  $printer -> text($this->Col4("TOTAL NO SUJETO",0 ,  Helpers::Format(0), 8,  Helpers::Format(0), 10, Helpers::Format(0), 10));
...
$printer -> text($this->DosCol("CREDITO FISCAL $: ", 20, Helpers::Format($t_credito), 5));
$printer -> text($this->DosCol("IMPUESTO $: ", 20, Helpers::Format($t_imp), 5));
$printer -> text($this->DosCol("TOTAL $: ", 20, Helpers::Format($total), 5));
...
$printer -> text($this->DosCol("ABONOS $: ", 20, Helpers::Format($abonos), 5));
...
$printer -> text($this->DosCol("EFECTIVO $: ", 20, Helpers::Format($t_efectivo), 5));
$printer -> text($this->DosCol("TARJETA DE CREDITO $: ", 20, Helpers::Format($t_tarjeta), 5));
$printer -> text($this->DosCol("TOTAL $: ", 20, Helpers::Format($total), 5));
...
// VENTA TOTAL - subtotal, iva, total
  ...
  $printer -> text($this->Col4("TOTAL GRAVADO",0 ,  $st, 12,  $im, 10, $to, 10));
  $printer -> text($this->Col4("TOTAL EXENTO",0 ,  Helpers::Format(0), 12,  Helpers::Format(0), 10, Helpers::Format(0), 10));
  $printer -> text($this->Col4("TOTAL NO SUJETO",0 ,  Helpers::Format(0), 10,  Helpers::Format(0), 10, Helpers::Format(0), 10));
...
$printer -> text($this->Col4("DOCUMENTO",0 ,  "", 0,  "CANTIDAD", 20, "TOTAL", 15));
$printer -> text($this->Col4("TICKETS",0 ,  "", 0,  Helpers::Entero($cant_t), 18, Helpers::Format($t_ticket), 20));
$printer -> text($this->Col4("FACTURAS",0 ,  "", 0,  Helpers::Entero($cant_f), 17, Helpers::Format($t_factura), 20));
$printer -> text($this->Col4("CREDITO FISCAL",0 ,  "", 0,  Helpers::Entero($cant_c), 11, Helpers::Format($t_credito), 20));

$printer->feed();
$printer -> text("DOCUMENTOS ANULADOS");
$printer->feed();
$printer -> text("_______________________________________________");
$printer->feed();
...
if($t_ticket > 0){
  ...
  $printer -> text("TICKETS"); 
  $printer->feed();
  $printer -> text($this->Col4("HORA - TICKET", 0,  "SUBTOTAL", 10,  "IVA", 10, "TOTAL", 10));

  $a = $db->query("SELECT num_fac, hora FROM ticket_num WHERE edo = 2 and tipo = 1 and td = ".$_SESSION["td"]." and fecha = '$fechax'");
      $cant_t = $a->num_rows;
      $subTotalAnulado = 0;
      $impAnulados = 0;
      $totalAnulado = 0 ;
      foreach ($a as $b) {
          if ($r = $db->select("sum(stotal), sum(imp), sum(total)", "ticket", "WHERE num_fac = '".$b["num_fac"]."' and tipo = 1 and td = ".$_SESSION["td"]."")) { 
              ...
              $impt = $r["sum(imp)"];
              $totalt = $r["sum(total)"];
              $subTotalAnulado = $subTotalAnulado + $r["sum(stotal)"];
              $impAnulados = $impAnulados+ $r["sum(imp)"];
              $totalAnulado = $totalAnulado + $r["sum(total)"];
          } unset($r);  

  $printer -> text($this->Col4($b["hora"] . " - " . $b["num_fac"],0 ,  $stotalt, 10,  $impt, 10, $totalt, 10));
  }  $a->close();

  $printer -> text($this->Col4(" TOTAL ", 10 ,  $subTotalAnulado, 10,  $impAnulados, 10, $totalAnulado, 10));
...
if($t_factura > 0){
  ...
  $printer -> text("FACTURAS");
  $printer->feed();
  $printer -> text($this->Col4("HORA - FACTURA", 0,  "SUBTOTAL", 10,  "IVA", 10, "TOTAL", 10));

  $a = $db->query("SELECT num_fac, hora FROM ticket_num WHERE edo = 2 and tipo = 2 and td = ".$_SESSION["td"]." and fecha = '$fechax'");
      $cant_f = $a->num_rows;
      foreach ($a as $b) {
  ...
  $printer -> text($this->Col4($b["hora"] . " - " . $b["num_fac"],0 ,  $stotalp, 10,  $impp, 10, $totalp, 10));
...
if($t_credito > 0){
  ...
  $printer -> text("CREDITO FISCAL");
  $printer->feed();
  $printer -> text($this->Col4("HORA - CCF", 0,  "SUBTOTAL", 10,  "IVA", 10, "TOTAL", 10));

  $a = $db->query("SELECT num_fac, hora FROM ticket_num WHERE edo = 2 and tipo = 3 and td = ".$_SESSION["td"]." and fecha = '$fechax'");
    $cant_c = $a->num_rows;
      foreach ($a as $b) {
  ...
  $printer -> text($this->Col4($b["hora"] . " - " . $b["num_fac"],0 ,  $stotalc, 10,  $impc, 10, $totalc, 10));
  }  $a->close();
...
$printer->close();
 ...
 public function Col4($col1, $esp1,  $col2, $esp2, $col3, $esp3,  $col4,$esp4){
    ...
    public function Item2($cant,  $name = '', $price = '', $total = '', $dollarSign = false){
      $rightCols = 8;
      $leftCols = 28;
      if ($dollarSign) {
          $leftCols = $leftCols / 2 - $rightCols / 2;
...
public function DosCol2($izquierda = '', $iz, $derecha = '', $der){
      $left = str_pad($izquierda, $iz, ' ', STR_PAD_LEFT) ;
      $right = str_pad($derecha, $der, ' ', STR_PAD_LEFT);
      return "$left$right\n";
  }
...
public function Col42($col1, $esp1,  $col2, $esp2, $col3, $esp3,  $col4,$esp4){
      $la1 = str_pad($col1, $esp1, ' ', STR_PAD_LEFT);
      $la2 = str_pad($col2, $esp2, ' ', STR_PAD_LEFT);
      $la3 = str_pad($col3, $esp3, ' ', STR_PAD_LEFT);
      $la4 = str_pad($col4, $esp4, ' ', STR_PAD_LEFT);
