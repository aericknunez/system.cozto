...
if ($seslog->login_check() == TRUE) {
   ...
   $a = $db->query("SELECT * FROM ticket WHERE orden = '".$_REQUEST["orden"]."' and td = ".$_SESSION["td"]." and tx = ".$_SESSION["tx"]."");
...
// datos generales de la factura
    if ($r = $db->select("fecha, hora, efectivo, num_fac, tipo", "ticket_num", "WHERE orden = '".$_REQUEST["orden"]."' and td = ".$_SESSION["td"]." and tx = ".$_SESSION["tx"]."")) { 
        $fecha = $r["fecha"];
        $hora = $r["hora"];
        $efectivo = $r["efectivo"];
        $factura = $r["num_fac"];
    ...
    if ($r = $db->select("sum(stotal) as stotal, sum(descuento) as descuento, sum(imp) as imp, sum(total) as total", "ticket", "WHERE orden = '".$_REQUEST["orden"]."' and td = ".$_SESSION["td"]." and tx = ".$_SESSION["tx"]."")) { 
        $stotal = $r["stotal"];
        $total = $r["total"];
        $imp = $r["imp"];
...
/* 
    ...
    $cliente = Helpers::ClienteProducto($_REQUEST["orden"], $tipo, $factura, $time);
            ...
            <div id="receipt-data">
                <div>
                    <div style="text-align:center;">
                        ...
                        <h1 style=" margin-bottom: 0;">GRAVITY</h1>
                        <div style="text-align: center;">BIKES AND MORE</div>
                    ...
                    <hr style="width: 100%; border-bottom: 1px solid #ddd; margin-bottom: 0;">
                    <div class="letras" style="text-align: right;">
                         Fecha: <?php echo $fecha . " | " . $hora ?></div>
                    <p class="letras" style=" margin-bottom: 0;" >Factura: <strong><?php echo str_pad($factura, 8, "0", STR_PAD_LEFT); ?></strong></p>
                    <?php if($cliente != NULL){
                        echo '<p class="letras">
                            Cliente: '. $cliente .'
                        </p>';                     
