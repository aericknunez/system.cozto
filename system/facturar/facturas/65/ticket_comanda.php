...
<?php //datos del cliente 
    ...
    if ($r = $db->select("nombre, codigo, direccion, municipio, telefono, comentarios", "clientes", "WHERE hash = '".$cliente."' and td = ".$_SESSION["td"]."")) { 
            $nombre = $r["nombre"];
...
?>
                    <h1 class="text-center">Distribuidora López</h1><br> 
                    <div style="text-align: right; margin-top: -20px;">
                    Fecha: <?php echo date("d-m-Y") ?></div>
                    <p style="padding-top: -20px;">Orden N°: <strong><?php echo str_pad($_REQUEST["orden"], 8, "0", STR_PAD_LEFT); ?></strong></p>
                    <?php echo "Cliente: ".$nombre; ?><br>
                    <?php echo "Codigo: ". $codigo; ?><br> 
                    <?php echo "Dirección: ".$direccion; ?>
              <!--  <?php echo $municipio; ?><br>-->
                    <?php echo $comentarios; ?><br> 
                    <?php echo "Teléfono: ". $telefono; ?><br> 
                    <?php echo "Vendedor : " .Helpers::GetUser("login_userdata", "nombre", "user", $b["user"]); ?>
